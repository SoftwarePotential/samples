...
Partial Class SpAgent
    ...
    Partial Private Shared Sub ConfigureNamedUserDiscovery(configure As Action(Of Func(Of String)))
    End Sub
    ...
    Private Shared Sub ConfigureDistributor(configure As Action(Of Func(Of IAgentDistributorsConfigurationPhase, IAgentEmbeddedLicensesConfigurationPhase)))
        ...
        Dim namedUserCallback As Func(Of String) = Nothing
        SpAgent.ConfigureNamedUserDiscovery(Sub(resolveNamedUser As Func(Of String))
                                                namedUserCallback = resolveNamedUser
                                            End Sub)
        If namedUserCallback Is Nothing Then
        ...
        Else
            configure(Function(agent As IAgentDistributorsConfigurationPhase) agent.WithDistributor(Function(distributor As IDistributorCommenceConfigurationPhase) distributor.WithDiscovery(callback).WithNamedUser(namedUserCallback).CompleteWithDefaults()))
        End If
    End Sub
