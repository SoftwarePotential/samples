...
namespace Sp.Agent
{
    ...
    static partial class SpAgent
    {
        ...
        /// <summary>
        /// <para>Partial method enabling specification of an appropriate License Storage Policy for Local hardware bound licenses (excluding Removable stores) by a partnering partial class.</para>
        /// <para>Typically an implementation of this is provided via a SoftwarePotential.Configuration.* package.</para>
        /// <para>Failure to provide an implementation of this partial method and/or <code>ConfigureRemovableStorage()</code> that correctly invokes the <paramref name="configure"/> 
        /// callback (or a <code>ConfigureLocalStorage()</code> implementation) will result in an <see cref="InvalidOperationException"/> at Application startup.</para>
        /// </summary>
        ...
        /// <para><example>See the code emitted by any of the following <c>SoftwarePotential.Configuration.</c> packages: <c>Local.*</c>, <c>Web</c> or <c>Distributor</c>.</example></para>
        /// </param>
        static partial void ConfigureHardwareBoundStorage( Action<Func<IAgentHardwareBoundStorageConfigurationPhase, IAgentExternalStorageConfigurationPhase>> configure );
        ...
        /// <para><example>See the code emitted by any of the following <c>SoftwarePotential.Configuration.</c> packages: <c>Local.*</c>, <c>Web</c> or <c>Distributor</c>.</example></para>
        /// </param>
        static partial void ConfigureRemovableStorage( Action<Func<IAgentExternalStorageConfigurationPhase, IAgentCloudStorageConfigurationPhase>> configure );


        /// <summary>
        /// <para>Partial method enabling specification of an appropriate License Storage Policy for licenses to be maintained on Cloud stores by a partnering partial class.</para>
        /// <para>Typically this is tailored by the ISV to reflect the particular discovery policies/hardware integration dictated by one's licensing scheme/technological constraints.</para>
        /// <para>Implementation is optional (assuming you have a <code>ConfigureLocalBoundStorage()</code> implementation). Can NOT be combined with <code>ConfigureLocalStorage()</code>.</para>
        /// </summary>
        /// <param name="configure">
        /// <para>delegate that accepts the Cloud Store Configuration segment of a <c>IAgentContext.Configure()</c> Fluent Configuration sequence.</para>
        /// <para><example>See the code emitted by any of the following <c>SoftwarePotential.Configuration.</c> packages: <c>Local.*</c>, <c>Web</c> or <c>Distributor</c>.</example></para>
        /// </param>
        static partial void ConfigureCloudStorage( Action<Func<IAgentCloudStorageConfigurationPhase, IAgentDistributorsConfigurationPhase>> configure );
        ...
        {
            var configureLocalBoundStorage = default( Func<IAgentCommenceConfigurationPhase, IAgentHardwareBoundStorageConfigurationPhase> );
            ConfigureLocalBoundStorage( configure => configureLocalBoundStorage = configure );
            var configureHardwareBoundStorage = default( Func<IAgentHardwareBoundStorageConfigurationPhase, IAgentExternalStorageConfigurationPhase> );
            ConfigureHardwareBoundStorage( configure => configureHardwareBoundStorage = configure );
            var configureRemovableStorage = default( Func<IAgentExternalStorageConfigurationPhase, IAgentCloudStorageConfigurationPhase> );
            ConfigureRemovableStorage( configure => configureRemovableStorage = configure );
            var configureCloudStorage = default( Func<IAgentCloudStorageConfigurationPhase, IAgentDistributorsConfigurationPhase> );
            ConfigureCloudStorage( configure => configureCloudStorage = configure );
            ...
            if ( configureLocalBoundStorage != null || configureRemovableStorage != null || configureHardwareBoundStorage != null || configureCloudStorage != null )
                if ( configureStorage != null )
                    throw CreateAmbiguousPartialMethodException();
                else
                {
                    if ( configureLocalBoundStorage == null )
                        configureLocalBoundStorage = x => x.DisableLocalStore();
                    if ( configureHardwareBoundStorage == null )
                        configureHardwareBoundStorage = x => x.DisableHardwareBoundStorage();
                    if ( configureRemovableStorage == null )
                        configureRemovableStorage = x => x.DisableRemovableStorage();
                    if ( configureCloudStorage == null )
                        configureCloudStorage = x => x.DisableCloudStorage();
                    configureStorage = commence => configureCloudStorage( configureRemovableStorage( configureHardwareBoundStorage( configureLocalBoundStorage( commence ) ) ) );
                }
