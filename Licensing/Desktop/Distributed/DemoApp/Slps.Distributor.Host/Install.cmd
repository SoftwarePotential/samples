...
ECHO ====================================================
ECHO Usage is:
ECHO %0 -[stop ^| start ^| install ^| uninstall ^| obliterate]
ECHO ====================================================
GOTO :EXITERROR
...
IF DEFINED REMOVEKEY (
	ECHO.
 	ECHO Ready to remove License Store...
	ECHO Removing License Store will delete all installed Distributor licenses...
	ECHO
	@ECHO OFf
	SET /P AREYOUSURE=Are you sure you want to delete Distributor license store?(Y/[N])
	IF /I "%AREYOUSURE%" NEQ "Y" GOTO DONE
	ECHO.
 	ECHO Deleting License Store on %BITS% bit OS ...
	ECHO Deleting License Store on %BITS% bit OS ... >> %LOGFILE% 2>&1
	ECHO
	IF %BITS%==64 (
		REG DELETE "HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\InishTech" /f >> %LOGFILE% 2>&1
	)
	IF %BITS%==32 (
		REG DELETE "HKEY_LOCAL_MACHINE\Software\InishTech" /f  >> %LOGFILE% 2>&1
		)
	IF ERRORLEVEL 1 (
		ECHO =============================
		ECHO Failed to delete license store
		ECHO =============================
		GOTO :EXITERROR
	)
	ECHO ...License Store Deleted.
	ECHO ...License Store Deleted. >> %LOGFILE% 2>&1
)
