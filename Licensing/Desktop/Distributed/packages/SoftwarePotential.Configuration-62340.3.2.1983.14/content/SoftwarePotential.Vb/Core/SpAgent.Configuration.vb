...
Partial Class SpAgent
    ...
    ''' <para><example>See the code emitted by any of the following <c>SoftwarePotential.Configuration.</c> packages: <c>Local.*</c>, <c>Web</c> or <c>Distributor</c>.</example></para>
    ''' </param>
    Partial Private Shared Sub ConfigureLocalBoundStorage(configure As Action(Of Func(Of IAgentCommenceConfigurationPhase, IAgentExternalStorageConfigurationPhase)))
    End Sub
    ...
    Private Shared Function FetchStorageConfigurationSequenceOrThrow() As Func(Of IAgentCommenceConfigurationPhase, IAgentDistributorsConfigurationPhase)
        Dim configureLocalBoundStorage As Func(Of IAgentCommenceConfigurationPhase, IAgentExternalStorageConfigurationPhase) = Nothing
        SpAgent.ConfigureLocalBoundStorage(Sub(configure As Func(Of IAgentCommenceConfigurationPhase, IAgentExternalStorageConfigurationPhase))
                                                  configureLocalBoundStorage = configure
                                              End Sub)
        Dim configureRemovableStorage As Func(Of IAgentExternalStorageConfigurationPhase, IAgentDistributorsConfigurationPhase) = Nothing
        Dim configureStorage As Func(Of IAgentCommenceConfigurationPhase, IAgentDistributorsConfigurationPhase) = Nothing
        If configureLocalBoundStorage IsNot Nothing OrElse configureRemovableStorage IsNot Nothing Then
            ...
            If configureLocalBoundStorage Is Nothing Then
                configureLocalBoundStorage = (Function(x As IAgentCommenceConfigurationPhase) x.DisableLocalBoundStorage())
            End If
            ...
            End If
            configureStorage = (Function(commence As IAgentCommenceConfigurationPhase) configureRemovableStorage(configureLocalBoundStorage(commence)))
        Else
